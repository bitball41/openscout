...
- Classifies each business's web presence with a large, **international** categorised domain index (social, link-in-bio, directory, booking, ordering, marketplace, Google profile, free site-builder, parked/for-sale).
- **Live-checks listed websites** from the browser across http/https and www variants to catch dead or parked domains — businesses whose site died still surface as leads.
- **Excludes national chains/franchises** (they have a corporate site, so they are not prospects) and **merges duplicate listings** of the same business so a shop listed twice is never reported as falsely siteless.
- Scores every lead with a **calibrated confidence %** and reports an **estimated mistake rate** for the surfaced set.
...
OpenScout treats lead detection as a confidence problem, not a yes/no guess. Five things keep the surfaced set's estimated mistake rate under 10%:

- **Classification.** `js/classify.js` holds a large, international categorised index of social, directory, booking, ordering, marketplace, link-in-bio, and free site-builder domains. A custom domain reads as a real website; a free `*.wixsite.com`/`business.site`/Linktree/Yelp-only presence reads as a lead. Non-web values (`tel:`, `mailto:`, bare phone numbers) read as "no website".
- **Chain exclusion.** Recognised national chains/franchises are dropped — a franchise location has a corporate website even when its profile omits one, so counting it is a mistake. The brand list is deliberately conservative to avoid excluding independent shops.
- **Duplicate merge.** The same business found across overlapping search tiles is merged, keeping the *strongest* web presence found. A business listed once with a site and once without therefore reads as "has a website", not a false lead.
- **Live verification.** `js/verify.js` probes each listed website directly from the user's browser (`no-cors` requests plus icon loads) across http/https and www/apex variants. A site is only marked offline when **every** connection attempt fails at the network level, so live-but-slow, https-only, or icon-less sites are never mislabelled. Dead/parked sites are reclassified as leads.
- **Calibrated confidence.** Each lead gets a 0–99 confidence from its classification certainty, the live-check result, establishment signals (reviews, phone, operating status), and a light business-type prior. The app surfaces the mean as an "estimated accuracy", and the Match-precision control filters the least-certain guesses. In Balanced mode the estimated mistake rate stays under 10%; Strict is tighter still.
